...
use para_random_sample, only: para_accprej_gaus1d2v_fulgyro_unifield_per_per, &
                              para_accept_reject_gaussian1d_ful2d_per_per
...
!!! initialize parameter_2d_sets
    ...
    pic2d%para2d%numparticles=50
    pic2d%para2d%numcircle = 4
    pic2d%para2d%sigma = 1.0
    pic2d%para2d%tempt = 1.0
    pic2d%para2d%dtful=0.0 ! (pic2d%para2d%dtgy)/real(pic2d%para2d%num_time,8)
...
!  call borissolve(pic2d)
!   call fulrk4solve(ful2dtmp,pic2d)
  
    call MPI_FINALIZE(IERR) 
  end program test_para_orbit_one
