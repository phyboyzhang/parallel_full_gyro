...
!!! pic orbit
    ...
    class(ful2dsend_Node),dimension(:), pointer :: ful2dsendtmp
...
num_p=0
  ...
  call mpi2d_alltoallv_box_per_per(row,comm,rank,pic2d%para2d%numproc, &
       pic2d%field2d%ep_weight,pic2d%field2d%epwg_w,pic2d%field2d%epwg_e,pic2d%field2d%epwg_n, &
       pic2d%field2d%epwg_s, pic2d%field2d%epwg_sw,pic2d%field2d%epwg_se, & 
       pic2d%field2d%epwg_nw,pic2d%field2d%epwg_ne,boxindex)
                                
!  x=(/1.0,2.0/)  
!  v=(/1.0,2.0/)
!  call new_position_per_per_ful(x,v,pic2d)
!
!print*, "rank=",rank, "x=",x,"v=",v
  ful2dtmp=>pic2d%ful2d_head
  allocate(ful2dsendtmp(0:size-1))
  do i=0, size-1
    allocate(ful2dsendtmp(i)%ptr)
  end do
  call para_accept_reject_gaussian1d_ful2d_per_per(ful2dtmp,pic2d)
 
  call push_particle_among_box_ful2d_per_per(ful2dsendtmp,pic2d,num_p)

!  call partition_density_to_grid_ful(pic2d)
 
!  call new_list_ful2d_per_per(pic2d)


    call MPI_FINALIZE(IERR) 
  end program test_para_pic
