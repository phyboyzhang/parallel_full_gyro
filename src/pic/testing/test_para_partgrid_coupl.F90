Program test_para_partgrid_coupl
...
!  call para_accprej_gaus1d2v_fulgyro_unifield_per_per(pic2d)  
...
!   
!   call para_write_file_2d(pic2d%field2d%den,fileitem1,rootdata,pic2d)
!
!   call close_file(fileitem1,rank)

!   length(1)=pic2d%layout2d%box(rank)(2)-pic2d%layout2d%box(rank)(1)+1
!   length(2)=pic2d%layout2d%box(rank)(4)-pic2d%layout2d%box(rank)(3)+1
    num_p=0
...
print*, "rank1=",rank1
            ...
            if(rank1==rank) then
              curful%coords(1:4)=coords(1:4)
              allocate(curful%next)
              curful=>curful%next
            else              
   ...
   curful=>pic2d%ful2d_head
   call mpi2d_alltoallv_send_particle_2d(curful,currk,num_p,pic2d)
...
!  call close_file(fileitem1,rank)
  ...
  end program  test_para_partgrid_coupl
